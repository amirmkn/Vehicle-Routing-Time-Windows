...
// Checks if two solutions are equal by comparing each route.
// (Assumes the overloaded == for Route is available.)
...
// Checks whether a candidate solution is tabu (i.e. appears in the tabu list).
...
//   tabu_tenure: number of iterations for which a solution stays in the tabu list.
Solution tabu_search(Solution initial, int max_iter, int tabu_tenure) {
    ...
    vector<TabuEntry> tabu_list;  // Tabu list of previously visited solutions

    // Open log file for appending iteration logs.
    ...
    while (!time_or_eval_limit_reached() && iter < max_iter) {
        ...
        // If candidate is tabu and does not meet the aspiration criterion (i.e. it is not better than best),
        // then skip it by generating another candidate.
        // In practice one might generate a full neighborhood and then choose the best non-tabu move.
        // Here we attempt a simple retry up to a fixed number of times.
        int trials = 0;
        ...
        // Logging details: log whether a new (or tabu) candidate was accepted.
        if (is_different(candidate, current))
            logFile << "Iter " << iter << ": New candidate generated with cost " << candidate_cost << endl;
        else
            logFile << "Iter " << iter << ": Candidate same as current solution. No change." << endl;

        // Accept candidate if it is better than the current one.
        // Otherwise (if worse), you can choose the best among all moves or simply use the candidate if it is not tabu.
        // Here we use a simple best-improvement strategy.
        if (candidate_cost < current_cost) {
            current = candidate;
            current_cost = candidate_cost;
        } else {
            // For diversification, you might also accept a worse candidate
            // if it is not tabu; here we simply set the candidate as current.
            current = candidate;
            current_cost = candidate_cost;
        }
        ...
        if (current_cost < best_cost) {
            ...
            logFile << ">>> New best found with cost " << best_cost << endl;
        }

        // Add the new current solution into the Tabu list.
        // (Deep copy current solution)
        tabu_list.push_back({current, tabu_tenure});

        // Decrease tabu tenure of each entry and remove expired ones.
    ...
    logFile << "Tabu Search completed after " << iter << " iterations." << endl;
    logFile.close();
...
int main(int argc, char* argv[]) {
    ...
    int tabu_tenure = 100;          // Tabu list size or tenure (typical range: 5–100)
    ...
    // Run Tabu Search
    Solution best = tabu_search(initial, max_iter, tabu_tenure);
